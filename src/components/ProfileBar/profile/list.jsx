...
const List = (props) => {
  ...
  const RemoveAList = async (listId) => {
    if(window.confirm("are you sure ?? remove this list !")){
        try {
  ...
  const AddNewList = async () => {
    if (newListNameInput && newListDescInput) {
      const resp = await AccountService.AddNewList(
        newListNameInput,
        newListDescInput
      );
      if (resp.success) {
        GetListItems();
        Modal.success({
          content: "your list addedd successfully to our database",
        });
        ...
        {showInputButton && (
          <div id="addNewListContainer">
            <Input
              ...
              className="InputAddNewList"
              type="text"
              placeholder="your list name"
            />
            <Input
              onChange={(e) => setNewListDescInput(e.target.value)}
              value={newListDescInput}
              className="InputAddNewList"
              type="text"
            ...
            <Badge count={item.item_count}>
              <Tooltip placement="bottom" title={item.description}>
                <Text
                  ...
                  onClick={() => props.listSelection(item.id)}
                >
                  {item.name}
                </Text>
              </Tooltip>
